...
```bash
$ java -jar wiremock-standalone-{{ wiremock_version }}.jar --enable-browser-proxying --port 9999
```
...
```bash
$ java -jar wiremock-standalone-{{ wiremock_version }}.jar --enable-browser-proxying --trust-proxy-target localhost --trust-proxy-target dev.mycorp.com
```

or if you're not interested in security you can trust all hosts:

```bash
$ java -jar wiremock-standalone-{{ wiremock_version }}.jar --enable-browser-proxying --trust-all-proxy-targets
```
