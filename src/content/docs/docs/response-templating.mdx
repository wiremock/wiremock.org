...
description: Generate dynamic API responses in WireMock using Handlebars templates. Access request data, use helpers, and create realistic mock responses with templating.
---

import CloudCallout from '../../../components/CloudCallout.astro';

<CloudCallout title="WireMock Cloud" icon="rocket">
    WireMock Cloud's web-based editor with embedded test tool makes response template development easy<br/>
...
As of WireMock version `4.0.0-beta.21` a new `jsonSort` helper has been introduced.  This allows you to specify a field
within an JSON array to sort by. The field is referenced using a JSON path expression, and all sort field values must be 
