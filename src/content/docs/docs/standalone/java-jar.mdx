...
<CloudCallout title="WireMock Runner" icon="seti:pipeline">
    WireMock Runner extends WireMock Cloud for hybrid environments. Run playback, recording, and mock management locally, in CI/CD pipelines, or behind firewalls — while keeping your mocks centrally managed in the cloud.
    [**Learn more about WireMock Runner >**](https://www.wiremock.io/product/runner?utm_source=oss-docs&utm_medium=oss-docs&utm_campaign=runner-callout-java-jar)
</CloudCallout>
...
<VCode lang="bash">
{`$ java -jar wiremock-standalone-${VERSIONS.WIREMOCK_STABLE}.jar`}
</VCode>
