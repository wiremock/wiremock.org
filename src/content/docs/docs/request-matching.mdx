...
description: Learn how to match HTTP requests in WireMock using URLs, headers, query parameters, request bodies, JSON, XML, cookies, and advanced matchers including regex, JSONPath, and XPath.
---
...
You can match numbers in their String representation using `equalTo`. For matching based on their numeric value, use `equalToNumber`, `greaterThanNumber`, `greaterThanOrEqualToNumber`, `lessThanNumber` and `lessThanOrEqualToNumber`.

If non-number values are provided either as expected or actual, the matcher report these as not matching. It can be used for matching both strings and numbers.
<Tabs>
<TabItem label="Java">
```java
        stubFor(get(urlPathEqualTo("/things"))
        .withQueryParam("id", greaterThan("1"))
        .willReturn(ok()));
        stubFor(get(urlPathEqualTo("/things"))
        .withQueryParam("id", greaterThan(1))
        .willReturn(ok()));
